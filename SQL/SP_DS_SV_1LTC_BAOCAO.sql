...
AS BEGIN
	BEGIN TRY
		...
		SELECT SV.MASV,  IIF(SV.PHAI=1,'Nữ','Nam') AS PHAI, SV.MALOP, SV.HO, SV.TEN INTO #DS_SV
		FROM SINHVIEN SV, DANGKY DK
		WHERE SV.MASV = DK.MASV AND DK.MALTC = 2

		-- GAN STT VA CHINH NOI DUNG CAN
		SELECT ROW_NUMBER() OVER(ORDER BY TEN, HO) AS STT, MASV, HO, TEN, PHAI, MALOP
		FROM #DS_SV
