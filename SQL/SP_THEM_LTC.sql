...
AS BEGIN
	BEGIN TRY
		...
		IF(EXISTS(SELECT MALTC FROM LOPTINCHI WHERE NIENKHOA = @NIENKHOA AND HOCKY = @HOCKY AND MAMH = @MAMH AND NHOM = @NHOM))
			RAISERROR ('Thông tin bạn điền không đúng. Vui lòng kiểm tra lại!', 11, 1);
		ELSE 
